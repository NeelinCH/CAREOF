<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Nueva Tarea para') }}: {{ $planta->nombre }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('plantas.tareas.store', $planta->id) }}">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="tipo" name="tipo" required
                                    ...
                                    <option value="trasplante" {{ old('tipo') == 'trasplante' ? 'selected' : '' }}>Trasplante</option>
                            ...
                            <div>
                                ...
                                <input type="number" id="frecuencia_dias" name="frecuencia_dias" min="1" value="{{ old('frecuencia_dias', 7) }}" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-200 focus:ring-opacity-50">
                            ...
                            <div>
                                ...
                                <input type="date" id="proxima_fecha" name="proxima_fecha" value="{{ old('proxima_fecha', now()->format('Y-m-d')) }}" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-200 focus:ring-opacity-50">
                                @error('proxima_fecha')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="descripcion" name="descripcion" rows="3"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-200 focus:ring-opacity-50">{{ old('descripcion') }}</textarea>
                                @error('descripcion')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
